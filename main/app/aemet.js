function obtenerDatosMeteorologicos() {
  ...
  */
                    //
                    const ajaxData = [
                      { titulo: 'Datos registrados en su último acceso', valor: hora + ' horas'},
                      { titulo: 'Temperatura (ºC)', valor: datosPrediccionTemperatura }, 
                      { titulo: 'Humedad (%)', valor: datosPrediccionHumedad},
                      { titulo: 'Prob. lluvia (%)', valor: datosPrediccionProbPrecipitacion},
                      { titulo: 'Prob. tormenta (%)', valor: datosPrediccionProbTormenta}
                    ];
                    
                    // Obtén el contenedor de la lista
                    const dataList = document.getElementById('listaDatosAemet');

                    // Itera sobre los datos y crea elementos de lista para cada uno
                    ajaxData.forEach((dato) => {
                      const li = document.createElement('li');

                      // Crea el contenido del elemento de lista con título y valor
                      li.innerHTML = `
